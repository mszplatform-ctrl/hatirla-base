...
export function SpaceSelfie({ onBack }: SpaceSelfieProps) {
  ...
  /**
   * Fetch resultImage via backend proxy (avoids fal.ai CDN CORS restrictions),
   * draw on canvas, composite "xotiji.app" watermark bottom-left, and return
   * the result as a JPEG Blob.
   */
  async function getWatermarkedBlob(): Promise<Blob> {
    const proxyUrl = `${API_BASE}/api/proxy-image?url=${encodeURIComponent(resultImage!)}`;
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: 'My XOTIJI Space Selfie',
          text: `✨ My cosmic travel identity was generated by XOTIJI — ${link}`,
          url: link,
        });
        return;
      }
