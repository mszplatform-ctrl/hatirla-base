...
export function SpaceSelfie({ onBack }: SpaceSelfieProps) {
  ...
  async function handleShare() {
    if (!resultImage || !selectedScene) return;
    const shareText = `🚀 COSMIC IDENTITY GENERATED — ${selectedScene.label} ✨ ${tagline} #XOTIJI #SpaceSelfie`;
    if (navigator.share) {
      try {
        // Convert base64 data URL to Blob for file sharing
        const res = await fetch(resultImage);
        const blob = await res.blob();
        const file = new File([blob], `xotiji-space-selfie-${selectedScene.id}.jpg`, { type: blob.type || 'image/jpeg' });
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
          await navigator.share({ files: [file], title: 'XOTIJI Transmission', text: shareText });
        } else {
          // Share without file (older browsers that support share but not file share)
          await navigator.share({ title: 'XOTIJI Transmission', text: shareText, url: 'https://xotiji.app' });
        }
      } catch (err) {
      ...
      // Desktop fallback: copy tagline to clipboard
      navigator.clipboard?.writeText(tagline).catch(() => {});
        ...
        <div style={{ padding: '24px 24px 40px', textAlign: 'center', background: '#000' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '14px', marginBottom: '24px' }}>
            {[
              ...
              { icon: '↗', label: 'SHARE', action: handleShare,    bg: 'linear-gradient(135deg, #0f766e 0%, #0ea5e9 100%)', color: '#fff', border: 'none' },
            ].map(btn => (
